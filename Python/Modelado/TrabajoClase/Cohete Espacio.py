...
def Cohete(t,ci,G,Mt,ML,PosTierra,PosLuna):
    ...
    a=dvdtComponent(Mt,NaveTierra)#G*Mt*NaveTierra/DistanciaNaveTierra**3
    b=dvdtComponent(Mt,NaveTierra)#G*Mt*LunaTierra/DistanciaNaveLuna**3
    dvdt=a+b#GetVelo(ci[1],PosTierra,PosLuna)
    dxdt=ci[0]
...
def GetVeloInicial(MasaObj,ArrayDistance):
    ...
    return a
def GetVelo(PosCohete,posTierra,PosLuna):
    PosCoheteTierra = posTierra-PosCohete
    PosNaveLuna=PosLuna-PosCohete
    a=+dvdtComponent(MasaLuna,PosNaveLuna)
    b=+dvdtComponent(MasaTierra,PosCoheteTierra)
    return(a+b)
...
pl.plot(Y[:,1][:,0],Y[:,1][:,1],label="distancia recorrida en Z",color="black")
#"""
pl.legend()
pl.grid()